...
export class MacOSAppScanner {
  private static readonly DEFAULT_SEARCH_PATHS = ['/Applications', '$HOME/Applications'];
  ...
  public async scanApplications(): Promise<AppInfo[]> {
    try {
      ...
      const apps = await Promise.allSettled(
        appPaths.map(({ appPath, plistPath }) => this.processApplication(appPath, plistPath))
      );

      return apps
        .filter((result): result is PromiseFulfilledResult<AppInfo> => result.status === 'fulfilled')
   ...
   */
  public async getApplicationByBundleId(bundleId: string): Promise<AppInfo | null> {
   ...
   */
  public async getApplicationsByName(namePattern: string | RegExp): Promise<AppInfo[]> {
    const apps = await this.scanApplications();
  ...
  private async findApplicationPaths(): Promise<Array<{ appPath: string; plistPath: string }>> {
  ...
  private async processApplication(appPath: string, plistPath: string): Promise<AppInfo> {
    try {
      const plistData = await PlistParser.readPlist(plistPath);
      const { appName, bundleId, appIconName } = PlistParser.extractAppInfo(plistData);
      ...
      if (appIconName) {
        ...
        if (appIconPath && this.options.includeBase64Icon) {
          try {
            appIconBase64 = await IconProcessor.convertToBase64(appIconPath, this.options.iconSize);
          } catch (error) {
